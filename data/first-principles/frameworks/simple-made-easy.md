...
Simple Made Easy is Rich Hickey's framework for distinguishing **simple** (unentangled, comprehensible) solutions from merely **easy** (convenient, familiar) ones. Introduced in his Strange Loop 2011 talk "Simple Made Easy," the framework argues that many engineering problems stem from choosing what feels easy over what is objectively simple. For first principles thinking, this framework gives language and criteria for evaluating ideas beyond habit, fashion, or personal skill.
...
- **Ease axis**: Measures human proximity—what is close to hand, what libraries exist, what the team already knows.
...
## Why First Principles Thinkers Care

1. **Separates essence from accident**: First principles thinking seeks the irreducible pieces. Simplicity criteria provide tests for whether we've actually isolated those pieces.
2. **Prevents cargo-culting**: Many "industry standard" choices are easy because everyone already knows them. Evaluating them through the simplicity lens forces explicit reasoning instead of imitation.
3. **Supports recomposition**: Simple parts compose like LEGO bricks. Complex but easy solutions block recombination and experimentation.
4. **Aligns with reason over authority**: Simple designs can be explained, reasoned about, and verified. Complex-easy designs rely on experts and institutional memory.
...
4. **New tech evaluation**: Before adopting a tool, answer: "How does this keep things simple? Where does it make things merely easy?" Tie findings back to other frameworks (e.g., run a Five Whys on why a choice feels easy or use the Feynman Technique to explain the proposed design in plain terms).

## Takeaways

- Simplicity is an objective property of the solution; ease is a human perception of effort.
- First principles thinking demands we prioritize simplicity because it keeps reasoning grounded and composable.
...
Simple Made Easy is a commitment to unspectacular clarity. It favors boring, orthogonal parts that can be reasoned about independently. When decisions are anchored there, first principles thinking becomes a daily practice instead of an aspirational slogan.
